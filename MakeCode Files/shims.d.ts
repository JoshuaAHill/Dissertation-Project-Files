...
declare namespace sound {
    ...
    //% weight=80 shim=sound::__record
    function __record(): void;

    /**
     * Playback clip
     */
    //% blockID = audio_playback block="Playback sound clip"
    //% weight=70 shim=sound::__playback
    function __playback(): void;

    /**
     * Get Closest Primary Note
     */
    //% blockID = get_closest_note block="Get Closest Note"
    //% weight=60 shim=sound::__startRecording
    function __startRecording(): void;

    /**
     * Stop Recording
     */
    //% blockID = stop_recording block="Stop Recording"
    //% weight=60 shim=sound::__stopRecording
    function __stopRecording(): void;

    /**
     * Get Closest Primary Note
     */
    //% blockID = get_closest_Note block="Get Closest Note"
    //% weight=60 shim=sound::__getClosestNote
    function __getClosestNote(): int32;

    /**
     * Get Closest Primary Freq
     */
    //% blockID = get_closest_Freq block="Get Primary Freq"
    //% weight=60 shim=sound::__getClosestFreq
    function __getClosestFreq(): int32;

    /**
     * Get Closest Secondary Note
     */
    //% blockID = get_closest_Note_S block="Get Closest Secondary Note"
    //% weight=60 shim=sound::__getClosestSecondaryNote
    function __getClosestSecondaryNote(): int32;

    /**
     * Get Closest Secondary Freq
     */
    //% blockID = get_closest_Freq_P block="Get Secondary Freq"
    //% weight=60 shim=sound::__getClosestSecondaryFreq
    function __getClosestSecondaryFreq(): int32;

    /**
     * On Heard Note A
     */
    //% blockID = on_note_A block="On Hearing A"
    //% weight=70 shim=sound::__onNoteA
    function __onNoteA(body: () => void): void;

    /**
     * On Heard Note E
     */
    //% blockID = on_note_E block="On Hearing E"
    //% weight=70 shim=sound::__onNoteE
    function __onNoteE(body: () => void): void;

    /**
     * Play Freq
     */
    //% blockID = play_freq block="PLay Frequency"
    //% weight=70 shim=sound::__playFrequency
    function __playFrequency(freq: int32, ms: int32): void;

    /**
     * Start Morse
     */
    //% blockID = start_morse block="Start Morse"
    //% weight=60 shim=sound::__startRecognition
    function __startRecognition(): void;

    /**
     * Stop Morse
     */
    //% blockID = stop_morse block="Stop Morse"
    //% weight=60 shim=sound::__stopRecognition
    function __stopRecognition(): void;

    /**
     * Play Morse String
     */
    //% blockID = play_morse_string block="Play Morse Char"
    //% weight=60 shim=sound::__playString
    function __playString(str: string, primary: boolean): void;

    /**
     * Get last heard char
     */
    //% blockID = get_last_char block="Get last heard letter"
    //% weight=60 shim=sound::__getLastChar
    function __getLastChar(primary: boolean): int32;

    /**
     * Clear input buffers
     */
    //% blockID = clear_morse_buf block="Clear Buffer"
    //% weight=60 shim=sound::__clearStored
    function __clearStored(primary: boolean): void;

    /**
     * Get stored result
     */
    //% blockID = get_stored_morse_result block="Get Stored result"
    //% weight=60 shim=sound::__getStored
    function __getStored(primary: boolean): string;
}
declare namespace music {
